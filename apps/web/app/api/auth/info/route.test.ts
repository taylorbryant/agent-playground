...
describe("GET /api/auth/info", () => {
  ...
  test("returns local auth state with no GitHub connection", async () => {
    const { GET } = await routeModulePromise;
  ...
  test("treats a GitHub account as connected", async () => {
    githubAccount = { id: "gh-account-1" };
    const { GET } = await routeModulePromise;
  ...
  test("treats GitHub installations as connected", async () => {
